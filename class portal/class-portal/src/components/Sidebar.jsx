...
import { getFilteredNavigation, PERMISSIONS } from '../utils/rbac'
...
function Sidebar({ onToggle, isCollapsed: externalIsCollapsed }) {
  ...
  const roleBasedNavItems = getFilteredNavigation(user?.roles || []);
  
  // Add keywords for search functionality
  const navItems = roleBasedNavItems.map(item => ({
    ...item,
    keywords: [item.title.toLowerCase(), item.path.toLowerCase().replace(/[\/-]/g, ' ')]
  }));
  ...
  const handleSearch = (e) => {
    ...
    if (searchValue === '') {
      ...
      const filtered = navItems.filter(item => {
        const labelMatch = item.title.toLowerCase().includes(searchValue);

...
import { apiGet, apiPost, apiPutUriList } from '../../utils/api'
...
function AddStudent() {
  ...
  const onSubmit = async (data) => {
    ...
    console.log(data)
    try {
      const res = await apiPost("/student", {
        id: data.id,
        name: data.name,
        age: data.age,
        gender: data.gender,
      ...
      await apiPutUriList(`/student/${data.id}/batch`, data.batch)
  ...
  const fetchBatch = async () => {
    try {
      const data = await apiGet("/batch")
      setBat(data["_embedded"]["batches"])
    } catch (error) {
